...
ImageCopy:
	la a0, Buffer2                        ; dest = Buffer2;
	la a1, Buffer1                        ; src = Buffer1;
	li a2, VS_BUF_H*VS_BUF_H*2            ; size = VS_BUF_H * VS_BUF_H * 2;
	jal VS_Memcpy                         ; VS_Memcpy(dest,src,size);
	nop
	li a0, 34                             ; vram_x = 34;
	li a1, 120                            ; vram_y = 120;
	li a2, VS_BUF_W                       ; image_w = VS_BUF_W;
	li a3, VS_BUF_H                       ; image_h = VS_BUF_H;
	la t1, Buffer1                        ; data = Buffer1;
	sw t1, 16(sp)
	jal VS_DMAImageDataToVram             ; VS_DMAImageDataToVram(vram_x,vram_y,image_w,image_h,data);
	nop
	li a0, 162                            ; vram_x = 162;
	li a1, 120                            ; vram_y = 120;
	li a2, VS_BUF_W                       ; image_w = VS_BUF_W;
	li a3, VS_BUF_H                       ; image_h = VS_BUF_H;
	la t1, Buffer2                        ; data = Buffer2;
	sw t1, 16(sp)
	jal VS_DMAImageDataToVram             ; VS_DMAImageDataToVram(vram_x,vram_y,image_w,image_h,data);
	nop
main:
