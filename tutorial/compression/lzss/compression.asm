...
InitGPU:	
	...
	addi sp, -64
DecompressTexture:
	la a0, Texture                        ; dest = Texture;
	la a1, LZSS                           ; src = LZSS;
	addiu a2, a0, VS_TEXTURE_SIZE         ; end = dest + VS_TEXTURE_SIZE;
	li t0, 0                              ; flag_num = 0; 
LoadLZSSFlag:
...
UploadTextureToVram:
	...
	sw t1, 16(sp)
	jal VS_TransferIndexImageDataToVram   ; VS_TransferIndexImageDataToVram(vram_x,vram_y,image_w,image_h,image);
	nop
	...
	la t1, Texture                        ; image = Texture;
	sw t1, 16(sp)
	jal VS_DMAIndexImageDataToVram        ; VS_DMAIndexImageDataToVram(vram_x,vram_y,image_w,image_h,image);
	nop
FillScreen:
	li t0, VS_IO
...
VS_DMAIndexImageDataToVram:
	...
	blt a0, a1, CompleteIndexDMA ; if(size < dma_size) { goto CompleteDMA; }
	nop                  
vs_dma_align:
	andi t1, a0, $f           
	bnez t1, CompleteIndexDMA   ; if(!(size % 16)) { goto CompleteDMA; }
	nop
vs_dma_align_size:
	addiu a0, 15                ; size += 15;
CompleteIndexDMA:
