...
HandleEvent:
	jal VS_EnterCritialSection            ; VS_EnterCritialSection();
	nop 
	li a0, $f2000003                      ; class = 0xf2000003;
	li a1, 2                              ; spec = 2;
	li a2, $1000                          ; event_callback_mode = 0x1000;
	la a3, VS_IncrVBlank                  ; func = VS_IncrVBlank;
	jal VS_OpenEvent                      ; event = VS_OpenEvent(class,spec,event_callback_mode,func);
	nop
	sw v0, VBlankEvent                    ; *VBlankEvent = event;
	move a0, v0                           ; enable_event = event;
	jal VS_EnableEvent                    ; VS_EnableEvent(enable_event);
	nop
	jal VS_ExitCritialSection             ; VS_ExitCritialSection();
	nop
DrawVBlank:
	li a0, 5                              ; x = 5;
	li a1, 276                            ; y = 276;
	...
	nop
	la a0, VBlankString                   ; string = VBlankString;
	lw a1, VBlank                         ; int = *VBlank;
	jal VS_Int2String                     ; VS_Int2String(string,int);
	nop
	li a0, 80                             ; x = 80;
	li a1, 276                            ; y = 276;
	la a2, VBlankString                   ; string = VBlankString;
	jal VS_DrawString                     ; VS_DrawString(x,y,string);
	nop
BufferSwap:
	li t0, VS_IO
...
# Function: VS_IncrVBlank
# Purpose: Increments the VBlank Counter by 1 
VS_IncrVBlank:
	lw a0, VBlank ; vblank = *VBlank;
	nop 
	addiu a0, 1   ; vblank++;
	sw a0, VBlank ; *VBlank = vblank;
	jr ra 
	nop
